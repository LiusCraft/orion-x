// Package apikey 是 API Key 的领域层：key 串的生成 / 解析 / 摘要、scope 目录与预设、
// 授权判定、限流与计数聚合。设计见 docs/api-key-design.md §5.1。
//
// 边界：不 import gin、不 import internal/billing；与仓储相接只通过本包的 Store 接口
// 和 store.APIKey 这个记录类型。
package apikey
...
// Prefix 是 key 串的公开前缀。格式冻结，改它等于换一代凭证（§1 D3）。
const Prefix = "ox_sk_"

// key 串的形状：`ox_sk_<lookup>_<secret>_<crc>`，四段、下划线分隔（对齐 GitHub / Stripe）。
const (
	// lookupLength 是公开段长度（10 位 base58 ≈ 58.6 bit），落库 varchar(16)。
	lookupLength = 10
	// secretLength 是秘密段长度（32 位 base58 ≈ 187 bit）：离线爆破不可行，
	// 所以存 sha256 就够，不需要慢哈希（§1 D4）。
	secretLength = 32
	// crcLength 是校验段长度：CRC32 最大 4294967295，base58 六位（58^6 ≈ 3.8e10）够。
	crcLength = 6
	// rawLength 是完整串长度，Parse 的第一道闸。
	rawLength = len(Prefix) + lookupLength + 1 + secretLength + 1 + crcLength
)

// alphabet 是 base58（Bitcoin）字母表：去掉 0OIl 等易混字符，人工抄写时少出错。
const (
	alphabet     = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz"
	alphabetBase = uint64(len(alphabet))
)

// ErrInvalid 表示这把 key 不能用（形状不对，或查不到）。
var ErrInvalid = errors.New("apikey: invalid key")

// Generate 生成明文（仅此一次返回）与可落库的记录。明文不许再出现在别处，
// 唯一例外是创建响应。
...
// Parse 只做形状校验：前缀、分段、字符集、长度、CRC。不打库：畸形输入在这里就被拒掉，
// 单字符改动一定落在 8 bit 之内，CRC32 保证检出。
...
// LookupOf 取出公开段，取不到返回空串。只给日志用：排障时靠它定位哪把 key 在报错，
// 不用打印任何秘密。
...
// Mask 返回列表里给用户看的掩码：保留公开段，秘密段一律隐去。明文离开创建响应后
// 没有第二份，掩码也只能由公开段拼出来。
func Mask(lookup string) string {
	return Prefix + lookup + "••••"
}

// crcSuffix 对 `lookup + "_" + secret`（不含前缀）取 CRC32(IEEE)，左补零到 6 位
// base58。它只用于"不打库就拒掉畸形输入"，不承担安全职责。
