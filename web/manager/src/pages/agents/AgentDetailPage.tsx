...
interface BotConfig {
  ...
  llm: {
    ...
    thinking: {
      ...
      preserve_history: "default" | "none" | "all";
    };
  };
    ...
    if (c.asr?.model_id || c.llm?.model_id) {
      return {
        ...
        llm: {
          ...DC.llm,
          ...c.llm,
          thinking: { ...DC.llm.thinking, ...(c.llm?.thinking || {}) },
        },
...
export default function AgentDetailPage() {
  ...
  const setThinking = (p: Partial<BotConfig["llm"]["thinking"]>) =>
    setCfg((c) => ({
      ...c,
      llm: { ...c.llm, thinking: { ...c.llm.thinking, ...p } },
    }));
            ...
            <TabsContent value="basic" className="space-y-5 pt-1">
              ...
              <div className="border-t border-zinc-800 pt-4 space-y-3">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <Switch
                    checked={llm.thinking.mode === "enabled"}
                ...
                {llm.thinking.mode === "enabled" && (
                  <div className="flex flex-wrap items-end gap-5">
                    <Field label="思考强度">
                      <Select
                        ...
                        onValueChange={(v) =>
                          setThinking({
                            effort: (v ??
                              "default") as BotConfig["llm"]["thinking"]["effort"],
                          })
                        }
                      >
                    ...
                    <div className="flex items-center gap-2 pb-1">
                      <Switch
                        checked={llm.thinking.preserve_history === "all"}
                        onCheckedChange={(checked) =>
                          setThinking({
            ...
            <TabsContent value="mcp" className="space-y-4 pt-1">
              <div className="flex flex-wrap gap-2">
                <div className="relative flex-1">
                  <Search className="pointer-events-none absolute left-3 top-1/2 z-10 h-4 w-4 -translate-y-1/2 text-[hsl(var(--muted-foreground))]" />
                  <Input
                    type="text"
                    value={mcpSearch}
                ...
                <div className="inline-flex h-9 items-center gap-0.5 rounded-md border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-0.5">
                  {(["all", "bound", "unbound"] as const).map((f) => (
                    <button
                      key={f}
                      type="button"
                      onClick={() => setMcpFilter(f)}
                      className={`h-8 rounded-[4px] px-3 text-xs transition-colors ${
                        mcpFilter === f
              ...
              {allFiltered.length > mcpPageSize && (
                <div className="flex items-center justify-between mt-4">
                  ...
                  <div className="flex items-center gap-2">
                    <SimpleSelect
                      value={String(mcpPageSize)}
                      onValueChange={(value) => {
                        setMcpPageSize(Number(value));
                        setMcpPage(1);
                      }}
