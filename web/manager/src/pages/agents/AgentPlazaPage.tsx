...
export default function AgentPlazaPage() {
  ...
  const filtered = TEMPLATES.filter((t) => {
    ...
    const matchQuery =
      !query || t.name.includes(query) || t.desc.includes(query);
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {filtered.map((tpl) => (
              ...
              >
                {/* Icon */}
                <div
                  className={`w-10 h-10 rounded-xl bg-gradient-to-br ${tpl.color} flex items-center justify-center text-xl mb-3 shadow-lg`}
                >
                  {tpl.icon}
                </div>
