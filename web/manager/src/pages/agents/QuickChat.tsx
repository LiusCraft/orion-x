...
export default function QuickChat({ agentId, vadMode }: { agentId: string; vadMode: string }) {
  ...
  const addMsg = useCallback((role: 'user' | 'assistant', text: string, sub = '') => {
    setMsgs(p => { const id = ++msgIdRef.current; return [...p, { role, text, sub, id }] })
  ...
  const connect = useCallback(async () => {
    ...
    const params = new URLSearchParams({ 'protocol-version': '1', 'device-id': agentId, 'client-id': crypto.randomUUID() })
    ...
    ws.onmessage = ({ data }) => {
      ...
      if (type === 'hello') {
        ...
        setConnected(true)
        setConnecting(false)
        addMsg('assistant', isAutoMode ? '已连接，开始对话吧' : '已连接，按住麦克风按钮说话', 'system')
      } else if (type === 'stt' && msg.text) {
        addMsg('user', msg.text as string, '语音识别')
      } else if (type === 'tts' && msg.text && (msg.state as string) !== 'sentence_end') {
        addMsg('assistant', msg.text as string, 'TTS')
      }
    }

    ws.onerror = () => { setConnecting(false); addMsg('assistant', '连接失败', 'error') }
    ws.onclose = () => { stopMic(); setConnected(false); setConnecting(false); audioCtxRef.current?.close(); audioCtxRef.current = null }
  ...
  // Cleanup on unmount
  useEffect(() => { return () => { stopMic(); wsRef.current?.close() } }, [stopMic])
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-zinc-800">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-xs ${connected ? 'text-emerald-400' : connecting ? 'text-yellow-400' : 'text-zinc-500'}`}>
            ● {connected ? '已连接' : connecting ? '连接中...' : ''}
          </span>
          {sessionId && <span className="text-[10px] text-zinc-600 font-mono truncate max-w-[120px]">{sessionId.slice(0, 12)}</span>}
        </div>
        {connected && (
        ...
        {msgs.map(m => (
          <div key={m.id} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] px-3 py-2 rounded-xl text-sm leading-relaxed
              ...
              <p>{m.text}</p>
              {m.sub && m.sub !== 'system' && (
                <p className={`text-[10px] mt-1 ${m.role === 'user' ? 'text-violet-300/60' : 'text-zinc-500'}`}>{m.sub}</p>
              )}
            </div>
          </div>
        ))}
      ...
      <div className="border-t border-zinc-800 px-3 py-2.5 flex items-center gap-2">
        <Input value={text} onChange={e => setText(e.target.value)}
          ...
          placeholder={connected ? (isAutoMode ? '输入文本或直接说话...' : '输入文本或按住按钮说话...') : '请先连接'}
          disabled={!connected}
        ...
        <Button size="sm" disabled={!connected || !text.trim()} onClick={handleSendText}
        ...
        <Button size="sm" variant="outline" disabled={!connected} onClick={() => send({ type: 'abort' })}
