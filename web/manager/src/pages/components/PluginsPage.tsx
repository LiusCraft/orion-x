...
import { Puzzle, Plus, Globe, Trash2, Edit2, CheckCircle2, Lock, Key, Zap } from 'lucide-react'
...
export default function PluginsPage() {
  const [installedIds, setInstalledIds] = useState<Set<string>>(new Set(['p1', 'p3']))
  const [myPlugins, setMyPlugins] = useState<MyPlugin[]>(MOCK_MY_PLUGINS)
  const [addOpen, setAddOpen] = useState(false)
  const [form, setForm] = useState({ name: '', url: '', method: 'POST', authType: '无鉴权', authValue: '', desc: '' })
  ...
  const handleAdd = () => {
    ...
    setForm({ name: '', url: '', method: 'POST', authType: '无鉴权', authValue: '', desc: '' })
    setAddOpen(false)
  }
      ...
      <div className="border-b border-zinc-800/80 px-8 py-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-zinc-500 mt-0.5">插件本质是工具，由智能体在对话中自动调用</p>
          </div>
          <Button
      ...
      <div className="px-8 py-6">
        <Tabs defaultValue="market">
          <TabsList className="bg-zinc-900 border border-zinc-800 h-9 p-0.5 mb-6">
            ...
            <TabsTrigger value="mine" className="text-xs data-[state=active]:bg-zinc-800 data-[state=active]:text-white text-zinc-500 h-8 px-4">
          ...
          <TabsContent value="market">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {MARKET_PLUGINS.map((plugin) => {
                ...
                return (
                  <div key={plugin.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 hover:border-zinc-700 transition-all">
                    <div className="text-2xl mb-3">{plugin.icon}</div>
                    <p className="font-medium text-sm text-white mb-1">{plugin.name}</p>
                    <p className="text-xs text-zinc-500 leading-relaxed mb-3 line-clamp-2">{plugin.desc}</p>
                    ...
                    >
                      {isInstalled ? (
                        <span className="flex items-center gap-1.5"><CheckCircle2 className="w-3 h-3" />已安装</span>
                      ) : '安装'}
                    </Button>
                  </div>
                )
          ...
          <TabsContent value="mine">
            {myPlugins.length === 0 ? (
              <div className="flex flex-col items-center py-20">
                ...
                <p className="text-zinc-600 text-xs mt-1 mb-4">通过 HTTP API 接入你自己的工具</p>
                <Button onClick={() => setAddOpen(true)} className="bg-violet-600 hover:bg-violet-500 text-white h-8 px-4 text-xs gap-1.5">
                  <Plus className="w-3.5 h-3.5" />添加 HTTP 插件
                </Button>
              </div>
            ) : (
              <div className="space-y-3">
                {myPlugins.map((p) => (
                  <div key={p.id} className="bg-zinc-900 border border-zinc-800 rounded-xl px-5 py-4 flex items-center gap-4 hover:border-zinc-700 transition-all group">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-zinc-800 border border-zinc-700/50 text-zinc-400 font-mono">{p.method}</span>
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-zinc-800 border border-zinc-700/50 text-zinc-500 flex items-center gap-1">
                          <Lock className="w-2.5 h-2.5" />{p.authType}
                        </span>
                      </div>
                      <p className="text-xs text-zinc-500 font-mono truncate">{p.url}</p>
                    </div>
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="bg-zinc-900 border-zinc-800 text-white sm:max-w-lg">
          ...
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="space-y-1.5">
                <Label className="text-xs text-zinc-400 uppercase tracking-wide">Method</Label>
                <select
                  value={form.method}
            ...
            <div className="space-y-1.5">
              <Label className="text-xs text-zinc-400 uppercase tracking-wide">API URL</Label>
              <Input
                value={form.url}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label className="text-xs text-zinc-400 uppercase tracking-wide">鉴权方式</Label>
                <select
                  value={form.authType}
              ...
              {form.authType !== '无鉴权' && (
                <div className="space-y-1.5">
                  <Label className="text-xs text-zinc-400 uppercase tracking-wide">
                    {form.authType === 'Bearer Token' ? 'Token' : form.authType === 'API Key' ? 'API Key' : '密码'}
                  </Label>
                  <Input
                    type="password"
            ...
            <div className="space-y-1.5">
              <Label className="text-xs text-zinc-400 uppercase tracking-wide">描述（告诉智能体何时使用）</Label>
              <Input
                value={form.desc}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAdd} disabled={!form.name.trim() || !form.url.trim()} className="bg-violet-600 hover:bg-violet-500 text-white">
              添加插件
            </Button>
          </DialogFooter>
        </DialogContent>
