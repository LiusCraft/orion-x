...
const TARGET_MAP: Record<string, { label: string; cls: string }> = {
  ...
  user:   { label: "用户", cls: "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" },
}
...
export default function MemoryPage() {
  ...
  const loadDeviceList = useCallback(async (agentId: string, page: number, q?: string) => {
    ...
    try {
      const { data } = await memoryApi.listDevices(agentId, {
        page, page_size: DEVICE_PAGE_SIZE,
        q: q || undefined,
      })
  ...
  // ── 加载条目 ──
  const loadEntryList = useCallback(async (deviceId: string, page: number, q?: string) => {
  ...
  const toggleAgent = async (id: string) => {
    if (expandedAgent === id) {
      setExpandedAgent(null); setDevices([]); setExpandedDevice(null)
      return
    }
    setExpandedAgent(id)
    setExpandedDevice(null)
  ...
  const toggleDevice = async (id: string) => {
    if (expandedDevice === id) { setExpandedDevice(null); return }
      ...
      <div className="border-b border-zinc-800/80 px-8 py-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-zinc-500 mt-0.5">终端用户与智能体对话中积累的长期记忆</p>
          </div>
          <div className="flex items-center gap-2 text-sm text-zinc-500 shrink-0">
            <Brain className="w-4 h-4 text-zinc-600" />
            {agentTotal} 个智能体
        ...
        <div className="flex gap-2 mt-4 items-center">
          ...
          {searchScope === "device" && !expandedAgent && (
            <span className="text-xs text-zinc-500 shrink-0">请先展开一个智能体</span>
          )}
          {searchScope === "content" && !expandedDevice && (
            <span className="text-xs text-zinc-500 shrink-0">请先展开一个设备</span>
          )}
        </div>
      </div>
      ...
      <div className="px-8 py-6 space-y-4">
        {error && (
          <div className="flex items-center gap-2 text-xs text-red-400 bg-red-400/10 border border-red-400/20 rounded-lg px-4 py-2.5">
            <span>{error}</span>
            <Button onClick={() => { setError(""); loadAgents() }} className="ml-auto bg-red-500/20 hover:bg-red-500/30 text-red-400 h-7 px-3 text-xs">重试</Button>
          </div>
        )}
        ...
        {!loadingAgents && agents.length === 0 && (
          <div className="flex flex-col items-center py-20">
            ...
            <p className="text-zinc-400 text-sm">
              {searchScope === "agent" && searchText ? "没有匹配的智能体" : "暂无智能体"}
            </p>
            <p className="text-zinc-600 text-xs mt-1">创建智能体后，终端用户对话产生的记忆将自动存储</p>
          </div>
        )}
          ...
          <div className="space-y-2">
              ...
              return (
                <div key={agent.id} className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
                  ...
                  >
                    ...
                    <span className="text-xs text-zinc-500">{agent.total} 条</span>
                    <span className="ml-auto text-xs text-zinc-600">{agent.device_count} 设备</span>
                  </button>
                  ...
                  {agentExpanded && (
                    <div className="border-t border-zinc-800">
                      ...
                      {!loadingDevices && devices.length === 0 && (
                        <div className="px-5 py-6 text-center text-xs text-zinc-600">暂无设备</div>
                      )}
                      ...
                      {!loadingDevices && devices.length > 0 && (
                        <div>
                          {devices.map((device) => {
                            ...
                            return (
                              <div key={device.id}>
                                ...
                                >
                                  {devExpanded ? <ChevronDown className="w-3.5 h-3.5 text-zinc-600 shrink-0 ml-1" /> : <ChevronRight className="w-3.5 h-3.5 text-zinc-600 shrink-0 ml-1" />}
                                  <HardDrive className="w-3.5 h-3.5 text-zinc-500 shrink-0" />
                                  <span className="text-sm text-zinc-300">{device.name || shortId(device.id)}</span>
                                  {device.name && <span className="text-xs text-zinc-600 font-mono">{shortId(device.id)}</span>}
                                  <span className="text-xs text-zinc-500">{device.total} 条</span>
                                </button>
                                ...
                                {devExpanded && (
                                  <div>
                                    {loadingEntries ? (
                                      ...
                                      <div className="px-5 py-6 text-center text-xs text-zinc-600">暂无记忆条目</div>
                                    ) : (
                                      <div>
                                        {entries.map((mem) => {
                                          const t = TARGET_MAP[mem.target] || TARGET_MAP.memory
                                          return (
                                            <div key={mem.id} className="flex items-start gap-3 px-5 py-2.5 hover:bg-zinc-800/10 transition-colors group border-b border-zinc-800/20 last:border-0">
                                              <span className={`shrink-0 mt-0.5 text-[10px] px-1.5 py-0.5 rounded border ${t.cls}`}>{t.label}</span>
                                              <span className="flex-1 text-sm text-zinc-300 leading-relaxed break-words min-w-0">{mem.content}</span>
                                              <span className="shrink-0 text-[11px] text-zinc-600 font-mono mt-0.5 whitespace-nowrap">{mem.created_at}</span>
                                              <button
                                                onClick={() => handleDelete(mem.id)}
                                        ...
                                        {entryPages > 1 && (
                                          <div className="flex items-center justify-end gap-2 px-5 py-2 border-t border-zinc-800/30">
                                            <Pagination page={entryPage} totalPages={entryPages} onChange={toEntryPage} />
                                          </div>
                                        )}
                                      </div>
                                    )}
                          ...
                          {devicePages > 1 && (
                            <div className="flex items-center justify-end gap-2 px-5 py-2 border-t border-zinc-800/30">
                              <Pagination page={devicePage} totalPages={devicePages} onChange={toDevicePage} />
                            </div>
                          )}
                        </div>
                      )}
            ...
            {agentPages > 1 && (
              <div className="flex items-center justify-center gap-2 pt-2">
                <Pagination page={agentPage} totalPages={agentPages} onChange={(p) => loadAgents(p)} />
              </div>
            )}
          </div>
        )}
...
function Pagination({ page, totalPages, onChange }: { page: number; totalPages: number; onChange: (p: number) => void }) {
  return (
    <div className="flex items-center gap-1.5">
      <button onClick={() => onChange(Math.max(1, page - 1))} disabled={page <= 1} className="px-2 py-1 text-xs rounded bg-zinc-800 border border-zinc-700 text-zinc-300 hover:bg-zinc-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors cursor-pointer">上一页</button>
      <span className="text-xs text-zinc-500 px-1">{page} / {totalPages}</span>
      <button onClick={() => onChange(Math.min(totalPages, page + 1))} disabled={page >= totalPages} className="px-2 py-1 text-xs rounded bg-zinc-800 border border-zinc-700 text-zinc-300 hover:bg-zinc-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors cursor-pointer">下一页</button>
    </div>
  )
}
