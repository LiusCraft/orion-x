...
import { Layers, Plus, Trash2, Edit2, CheckCircle2, Eye, EyeOff, Shield } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs'
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter } from '@/components/ui/dialog'
import { Input } from '@/components/ui/input'
import { Label } from '@/components/ui/label'
import { modelApi, providerApi, type AIModel, type ModelType, type Provider } from '@/lib/api'
...
const TYPE_LABEL: Record<string, string> = {
  text: '文本', vision: '视觉', speech: '语音', multimodal: '全模态', embedding: '向量',
}
...
export default function MyModelsPage() {
  ...
  const [form, setForm] = useState<{
    provider_id: string; name: string; type: ModelType; base_url: string; model_id: string
  ...
  const filtered = activeType === 'all' ? models : models.filter((m) => m.type === activeType)
    ...
    <div className="min-h-full">
      ...
      <div className="px-8 py-6">
        {loading ? (
          ...
          <Tabs value={activeType} onValueChange={(v) => setActiveType(v as ModelType | 'all')}>
            <TabsList className="bg-zinc-900 border border-zinc-800 h-9 p-0.5 mb-6 gap-0">
              <TabsTrigger value="all" className="text-xs data-[state=active]:bg-zinc-800 data-[state=active]:text-white text-zinc-500 h-8 px-4">
                全部
                <span className="ml-1.5 text-[10px] text-zinc-600">({models.length})</span>
              </TabsTrigger>
              {modelTypes.map((value) => (
                <TabsTrigger key={value} value={value} className="text-xs data-[state=active]:bg-zinc-800 data-[state=active]:text-white text-zinc-500 h-8 px-4">
              ...
              <TabsContent key={value} value={value}>
                {filtered.length === 0 ? (
                  <div className="flex flex-col items-center py-20">
                    ...
                    <p className="text-zinc-400 text-sm">还没有{TYPE_LABEL[value] ?? ''}模型</p>
                    <p className="text-zinc-600 text-xs mt-1 mb-4">添加兼容 OpenAI 接口的{TYPE_LABEL[value] ?? ''}模型</p>
                    <Button
                      onClick={() => openAdd(value === 'all' ? undefined : value as ModelType)}
                      className="bg-violet-600 hover:bg-violet-500 text-white h-8 px-4 text-xs gap-1.5"
                    >
                      <Plus className="w-3.5 h-3.5" />添加{TYPE_LABEL[value] ?? ''}模型
                    </Button>
                  </div>
                ) : (
                      ...
                      <div key={model.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 hover:border-zinc-700 transition-all group">
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              <span className={`text-[10px] px-1.5 py-0.5 rounded border ${TYPE_BADGE[model.type]}`}>
                                {TYPE_LABEL[model.type]}
                              </span>
                              {model.type === 'speech' && (() => {
                                const slug = model.provider?.slug ?? ''
                                if (slug.startsWith('tts/')) return <span className="text-[10px] px-1.5 py-0.5 rounded border bg-pink-400/10 text-pink-400 border-pink-400/20">TTS</span>
                                if (slug.startsWith('asr/')) return <span className="text-[10px] px-1.5 py-0.5 rounded border bg-sky-400/10 text-sky-400 border-sky-400/20">ASR</span>
                                return null
                              })()}
                              <span className="text-[11px] text-zinc-500">{model.provider?.name}</span>
                            </div>
                          </div>
                          {!model.is_system && (
                        ...
                        <div className="space-y-1.5">
                          <div className="bg-zinc-800/60 rounded-lg px-3 py-2">
                            ...
                            <p className="text-xs text-zinc-300 font-mono truncate">{model.model_id}</p>
                          </div>
                          <div className="bg-zinc-800/60 rounded-lg px-3 py-2">
                            <p className="text-[10px] text-zinc-600 mb-0.5">Base URL</p>
                            <p className="text-xs text-zinc-500 font-mono truncate">
                              {model.base_url || model.provider?.base_url || '—'}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between mt-3">
                          ...
                          <span className="text-[11px] text-zinc-600 font-mono">{model.created_at.slice(0, 10)}</span>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="bg-zinc-900 border-zinc-800 text-white sm:max-w-lg">
          ...
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                >
                  {modelTypes.map((value) => (
                    <option key={value} value={value}>{TYPE_LABEL[value] ?? value}模型</option>
                  ))}
                </select>
              </div>
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs text-zinc-400 uppercase tracking-wide">厂商</Label>
              <select
              ...
              >
                ...
                {providers
                  .filter((p) => {
                    const isVoice = p.slug.startsWith('tts/') || p.slug.startsWith('asr/')
            ...
            <div className="space-y-1.5">
              <Label className="text-xs text-zinc-400 uppercase tracking-wide">
                Base URL <span className="text-zinc-600 normal-case ml-1">（留空用厂商默认）</span>
              </Label>
              <Input
                value={form.base_url}
                onChange={(e) => setForm((f) => ({ ...f, base_url: e.target.value }))}
                placeholder={providers.find((p) => p.id === form.provider_id)?.base_url ?? 'https://...'}
                className="bg-zinc-800 border-zinc-700 text-white placeholder:text-zinc-600 text-sm font-mono focus-visible:ring-violet-500"
              />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs text-zinc-400 uppercase tracking-wide">API Key <span className="text-zinc-600 normal-case ml-1">（留空用厂商 Key）</span></Label>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setAddOpen(false)} className="border-zinc-700 text-zinc-300 hover:bg-zinc-800 hover:text-white">
              取消
            </Button>
            <Button
              onClick={handleAdd}
              disabled={saving || !form.name.trim() || !form.model_id.trim() || !form.provider_id}
