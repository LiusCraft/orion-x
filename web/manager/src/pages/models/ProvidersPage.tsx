...
import { Building2, Plus, Trash2, Edit2, Eye, EyeOff, CheckCircle2, Shield } from 'lucide-react'
      ...
      <div className="border-b border-zinc-800/80 px-8 py-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-zinc-500 mt-0.5">管理 AI 服务厂商的接入配置和 API Key</p>
          </div>
          <Button
            onClick={openAdd}
      ...
      <div className="px-8 py-6">
        {loading ? (
          ...
          <div className="flex flex-col items-center py-20">
            ...
            <p className="text-zinc-600 text-xs mt-1 mb-4">添加厂商后即可在该厂商下创建模型</p>
            <Button onClick={openAdd} className="bg-violet-600 hover:bg-violet-500 text-white h-8 px-4 text-xs gap-1.5">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {providers.map((p) => (
              <div key={p.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 hover:border-zinc-700 transition-all group">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {p.is_system && (
                        <span className="flex items-center gap-0.5 text-[10px] px-1.5 py-0.5 rounded border bg-violet-600/15 text-violet-400 border-violet-500/20">
                          <Shield className="w-2.5 h-2.5" />官方
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-1.5 mt-1">
                      {(() => {
                        ...
                        return <>
                          <span className={`text-[10px] px-1.5 py-0.5 rounded border font-mono uppercase ${catColor[cat] ?? 'bg-zinc-700/40 text-zinc-400 border-zinc-600/30'}`}>{cat}</span>
                          {vendor && <span className="text-[10px] px-1.5 py-0.5 rounded border bg-zinc-800 text-zinc-400 border-zinc-700 font-mono">{vendor}</span>}
                        </>
                      })()}
                    </div>
                  </div>
                ...
                <div className="bg-zinc-800/60 rounded-lg px-3 py-2">
                  ...
                  <p className="text-xs text-zinc-400 font-mono truncate">{p.base_url}</p>
                </div>
                ...
                <div className="flex items-center justify-between mt-3">
                  ...
                  <span className="text-[11px] text-zinc-600 font-mono">{p.created_at.slice(0, 10)}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <DialogContent className="bg-zinc-900 border-zinc-800 text-white sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              <Label className="text-xs text-zinc-400 uppercase tracking-wide">Slug</Label>
              {editTarget ? (
                <Input
                  value={form.slug}
                  disabled
                  className="bg-zinc-800 border-zinc-700 text-white text-sm font-mono disabled:opacity-40"
                />
              ) : (
                <select
                  value={form.slug}
                ...
                >
                  <option value="">选择厂商类型</option>
                  {Object.entries(
                    slugOptions.reduce<Record<string, ProviderSlug[]>>((acc, s) => {
            ...
            <div className="space-y-1.5">
              <Label className="text-xs text-zinc-400 uppercase tracking-wide">
                API Key{editTarget && <span className="ml-1 text-zinc-600 normal-case">(留空不修改)</span>}
              </Label>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={handleSave}
              disabled={saving || !form.name.trim() || !form.slug.trim() || !form.base_url.trim()}
