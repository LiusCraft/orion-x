...
import { Wand2, Upload, Mic, CheckCircle2, Loader2, AlertCircle } from 'lucide-react'
    ...
    <div className="min-h-full">
      <div className="border-b border-zinc-800/80 px-8 py-5">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-0.5">上传参考音频，克隆专属音色</p>
        </div>
      </div>
            ...
            return (
              <div key={label} className="flex items-center gap-2">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold transition-all ${
                  isDone ? 'bg-emerald-500 text-white' : isActive ? 'bg-violet-600 text-white' : 'bg-zinc-800 text-zinc-500'
                }`}>
                  {isDone ? <CheckCircle2 className="w-4 h-4" /> : idx}
                </div>
                <span className={`text-xs ${isActive ? 'text-white font-medium' : isDone ? 'text-emerald-400' : 'text-zinc-600'}`}>{label}</span>
          ...
          <div className="space-y-4">
            <div
              ...
              className={`border-2 border-dashed rounded-xl p-12 text-center cursor-pointer transition-all ${
                dragging ? 'border-violet-500 bg-violet-600/5' : 'border-zinc-700 hover:border-violet-500/50 hover:bg-zinc-900'
              }`}
            >
              ...
              <p className="text-sm text-zinc-300 font-medium mb-1">拖拽音频文件到此处</p>
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
              <p className="text-xs font-semibold text-zinc-400 mb-3">录制建议</p>
        ...
        {step === 2 && (
          <div className="space-y-5">
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 flex items-center gap-3">
              ...
              <button onClick={() => setStep(1)} className="ml-auto text-xs text-zinc-500 hover:text-zinc-300 cursor-pointer transition-colors">
                重新上传
              </button>
            </div>

            <div className="space-y-1.5">
              <Label className="text-xs text-zinc-400 uppercase tracking-wide">音色名称 <span className="text-red-400">*</span></Label>
              <Input
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder="给你的音色取个名字"
                className="bg-zinc-900 border-zinc-800 text-white placeholder:text-zinc-600 focus-visible:ring-violet-500"
                autoFocus
              />
            </div>

            <div className="space-y-1.5">
              <Label className="text-xs text-zinc-400 uppercase tracking-wide">描述（可选）</Label>
              <Input
                value={desc}
            ...
            <div className="flex gap-3 pt-2">
              <Button variant="outline" onClick={() => setStep(1)} className="border-zinc-700 text-zinc-300 hover:bg-zinc-800 hover:text-white">
                上一步
              </Button>
              <Button
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-8 text-center">
            {cloneStatus === 'processing' && (
              <>
                ...
                <p className="text-xs text-zinc-500 mb-6">正在分析音频特征，提取声线模型，预计需要 1-3 分钟</p>
            ...
            {cloneStatus === 'done' && (
              <>
                ...
                <p className="text-xs text-zinc-500 mb-6">音色「{name}」已创建，可在已有音色中找到</p>
                <div className="flex gap-3 justify-center">
                  <Button variant="outline" onClick={() => { setStep(1); setCloneStatus('idle'); setProgress(0); setName(''); setDesc(''); setFileName('') }} className="border-zinc-700 text-zinc-300 hover:bg-zinc-800 hover:text-white">
                    再复刻一个
                  </Button>
