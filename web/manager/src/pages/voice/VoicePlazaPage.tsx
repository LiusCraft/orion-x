...
export default function VoicePlazaPage() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([voiceApi.listSystem(), languageApi.list()])
  ...
  const allEmotions = [
    ...new Set(voices.flatMap((v) => Object.keys(v.emotions ?? {}))),
  ].sort();
        ...
        {loading ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {filtered.map((voice, idx) => {
              const isPlaying =
                playing === voice.id ||
                (playing && playing.startsWith(voice.id + ":"));
                ...
                >
                  <div className="flex items-center gap-3 mb-3">
                    {voice.avatar_url ? (
                      ...
                      <div
                        className={`w-12 h-12 rounded-xl bg-gradient-to-br ${color} flex items-center justify-center text-2xl font-bold text-white shadow-lg`}
                      >
                        {voice.name[0]}
                      </div>
                    )}
                  ...
                  <div className="flex gap-2 relative">
                    <button
                      ...
                      onClick={(e) => {
                        ...
                        if (
                          voice.emotions &&
                          Object.keys(voice.emotions).length > 0
                        ) {
                    ...
                    {emotionPopup === voice.id && voice.emotions && (
                      <div
                        data-popup="menu"
                        onMouseLeave={() => setEmotionPopup(null)}
                      ...
                      >
                        ...
                        {Object.entries(voice.emotions).map(([key, val]) => {
                          ...
                          const previewUrl =
                            ...
                            typeof val === "object" &&
                            "preview_url" in val
                              ? (val as Record<string, unknown>).preview_url
                              : undefined;
                          return (
                            <button
                              key={key}
                              onClick={(e) => {
                                ...
                                if (previewUrl) {
                                  setPlaying(
                                    isEmotionPlaying ? null : emotionId,
                                  );
                                }
                              }}
                              disabled={!previewUrl}
